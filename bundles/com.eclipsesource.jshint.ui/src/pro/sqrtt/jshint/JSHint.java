...
import pro.sqrtt.jshint.internal.JSHintRunner;
import pro.sqrtt.jshint.internal.ProblemImpl;
import pro.sqrtt.json.JsonObject;
import pro.sqrtt.json.JsonValue;
...
public class JSHint {
	...
	private static final String DEFAULT_JSHINT_VERSION = "2.4.4.dw";
		...
		try {
			...
			if (globalsValue != null) {
				String globalsExpr = "globals = " + globalsValue.toString()
						+ ";";
				globals = context.evaluateString(scope, globalsExpr,
						"[globals]", 1, null);
			}
			configuration.remove("globals");
			String optionsExpr = "options = " + configuration.toString() + ";";
			options = context.evaluateString(scope, optionsExpr, "[options]",
					1, null);
	...
	public void file(String file, String ext) {
		ScriptableObject tmp;
		if (ext == null) {
			throw new NullPointerException("Extention is null");
		}
		try {
			tmp = (ScriptableObject) options;
			tmp.put("ext_file", (NativeObject) options, ext);
			tmp.put("name_file", (NativeObject) options, file);
		} finally {
		}
	}
	...
	private boolean checkCode(Context context, String code) {
		try {
			Object[] args = new Object[] { code, options, globals, extention };
			return ((Boolean) jshint.call(context, scope, null, args))
					.booleanValue();
		} catch (JavaScriptException exception) {
			String message = "JavaScript exception thrown by JSHint: "
					+ exception.getMessage();
			throw new RuntimeException(message, exception);
		} catch (RhinoException exception) {
			String message = "JavaScript exception caused by JSHint: "
					+ exception.getMessage();
			throw new RuntimeException(message, exception);
		}
	}
	...
	private static BufferedReader getJsHintReader()
		...
		// the actual version
		String name = "pro/jshint/jshint-" + DEFAULT_JSHINT_VERSION + ".js";
